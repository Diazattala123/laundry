...
<div class="col-md-12">
  <div class="card">
    <div class="card-header">
      <h4 class="card-title text-center">HISTORY / LOG TRANSAKSI</h4>
    </div>
    <div class="card-body">
      ...
      @if ($history->actionType == \App\Models\modelUmum::ACTION_UPDATE)
        @foreach($contexts->changed as $key => $context)
          @if($key != 'updated_at' || $key == 'kode_invoice')
          <div class="row">
            <div class="col-lg-6">
         ...
         @foreach ($contexts as $key => $context)
            @if ($key == 'kode_invoice')
                <div class="alert alert-success">
                  <span><b style="box-shadow: rgba(99, 99, 99, 0.2) 0px 2px 8px 0px;margin-right:10px;">{{$history->actionType}}</b> {{ 'New Data Transaksi, Dengan Code Invoice' . ' : ' .  $context }}</span>
                </div>
             @endif
         @endforeach
